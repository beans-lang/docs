...
- `Device` is a `pub interface` with a **default method** `describe()`. Any type
  implementing `Device` gets it for free — even a type defined in another
  package.
...
`main.b` calls `banner(...)` and uses `Cart` without importing anything — they
are in the same package. But `helpers.b` still writes its own
`import shop.money`, because imports are per file: each file names the packages
it uses.

## Where to go next

- [Create and run a project](/start/projects/) — the project layout rules.
- [Why it is called POT](/pot/why-pot/) — module paths, import paths, and package names.
- [Local packages and imports](/pot/local-packages/) — how import paths resolve.
- [Generics](/guide/generics/) and [Interfaces and inheritance](/guide/interfaces/) — the features used here.

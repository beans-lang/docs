---
title: std.reflect
description: Runtime type, member, annotation, value, construction, and call descriptors.
---
...
the [reflection guide](/guide/reflection/) for examples, ownership rules, and
limits.
...
Use the contextual `type_of(T)` builtin for a type known at compile time.
...
`value(item)` transfers its argument into the box. A checked `as?` copies a
